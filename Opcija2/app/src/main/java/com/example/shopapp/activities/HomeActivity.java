...
public class HomeActivity extends AppCompatActivity {
    ...
    private void setUpReceiver() {
        broadcastReceiver = new BroadcastReceiver() {
            private boolean isBlue = true; // Flag to track the current color state

            @Override
            public void onReceive(Context context, Intent intent) {
                if (Objects.equals(intent.getAction(), "CHECK_COLOR")) {
                    Button obojiButton = findViewById(R.id.oboji);

                    // Toggle between blue and pink background colors
                    if (isBlue) {
                        obojiButton.setBackgroundTintList(ContextCompat.getColorStateList(context, R.color.blue_300));
                    } else {
                        obojiButton.setBackgroundTintList(ContextCompat.getColorStateList(context, R.color.pink_300));
                    }
